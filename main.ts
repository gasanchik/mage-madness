let hardwareMode: boolean = false
scene.setBackgroundImage(img`
    ...
    dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd
`)
customTiles.loadLevel(0)

/*
customTiles.loadLevel()
console.log(customTiles.currentLoadedLevel.tilemapData.tilemap[0].tileIDData)
forever(function() {
    renderer.screenX += controller.dx()
    renderer.screenY += controller.dy()
})
game.onUpdateInterval(1000, function() {
    for (let i = 0; i < customTiles.currentLoadedTiles.length; i++) {
        let loadedTile: customTiles.tile = customTiles.currentLoadedTiles[i]
        if (loadedTile) {
            //console.log(customTiles.currentLoadedTiles.length)
            //if (!utilities.IsObjectOnScreen(loadedTile.tileXPixels, loadedTile.tileYPixels, tileSizePixels, tileSizePixels)) {
            //}
        }
    }
})
